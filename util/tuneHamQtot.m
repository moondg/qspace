...
% Wb,Aug18,18
  ...
  getopt('init',varargin);
     pflag=getopt('-p'); % plot
     bfac =getopt('bfac',1.5); % factor in beta = bfac/dE
  ...
  if ~isempty(Qop)
   ...
   % Qop = charge operator; by default,
   % initialize Qop to diagonal matrix carrying symmetry labels
...
% NB! there may be gaps etc. in the spectrum at higher energies
% de=max(abs(diff(ee(:,1))));
  de=zeros(1,nd);
  ...
  de=de(find(de));
% q=min(de); de=mean(de(find(de<2*q)));
  de=min(de);
...
% wblog('TST','using beta=%.4g',beta); 
  ...
  for j=1:nq
     ...
     for i=2:miter
        ...
        if w==1
         % first need to find bracket around Qtot
           if     all(dy>0), x2(i,j)=x2(i-1,j)+1;
           elseif all(dy<0), x2(i,j)=x2(i-1,j)-1;
           else w=2; end
        end
        if w==2
         % then zoom in by bisectioning
           if dy(1)*dy(2)<0
                x2(i,j)=mean(x2([i-1,i-2],j)); % midpoint
